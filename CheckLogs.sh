...
if test ! -e "$chkfile";then
    echo "#应用名称,日志文件,关键字(多关键字|隔开),重试次数,最大执行次数,启动命令,停止命令,是否触发邮件,发邮件脚本,邮件联系人" >$chkfile
fi
...
do_writelog() {
    case $1 in
    i|I)
        shift #shift命令表示参数向左偏移，后面可跟数字（数字大小在参数个数范围内）
...
(
    ...
    if test ! -e "$tmpsdir/$log_md5sum";then
        ...
        if [ $log_new_count -eq $log_old_count ];then
           ...
           #读取更新的日志
           log_content=$(tail -n +`expr $log_old_count + 1` $log_filename|head -n +$log_upd_count) # 从老数据开始读取文件，然后最新的文件
           oldIFS=$IFS
           IFS="|"
           count=0
           for i in $log_gjz;do
